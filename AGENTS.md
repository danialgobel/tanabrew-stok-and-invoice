...
3. **Perbarui Daftar Catatan Pembaruan (`highlights`)**:
   - Rangkum perubahan menjadi **3 hingga 5 poin ringkas & padat** agar pop-up di HP tetap proporsional dan tidak menumpuk terlalu panjang.
   - Berikan badge yang sesuai:
...
1. **Berlaku untuk Semua Role**: Muncul bagi `Owner`, `Admin`, `Staff`, maupun `Developer`.
2. **Berlaku di Semua Perangkat**: Tampil responsif di Smartphone (Android/iOS), Tablet, Laptop, dan Desktop.
3. **Berlaku di Semua Rute**: Akan otomatis muncul di halaman mana pun yang pertama kali dibuka oleh pengguna yang sudah login (baik halaman Beranda maupun direct link).
4. **Mencegah Pop-up Berulang**: Menggunakan `localStorage.getItem("tanabrew_seen_app_version")` sehingga setelah pengguna mengetuk *"Mengerti & Mulai Gunakan"*, pop-up tidak akan muncul lagi sampai Anda merilis versi baru berikutnya.
5. **Tersinkronisasi dengan Halaman Akun**: Teks versi di Halaman Akun ([src/pages/Akun.tsx](file:///d:/Downloads/tanabrew-stok-and-invoice-github/src/pages/Akun.tsx)) otomatis membaca dari `CURRENT_RELEASE.versionLabel` dan bisa diklik untuk membuka riwayat changelog kapan saja.
